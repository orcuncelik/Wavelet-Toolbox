...
class ClassificationPage(QMainWindow):
    ...
    def __init__(self):
        ...
        loadUi("../ui/Classification.ui", self)
        ...
        self.checkBox_randomForest.setChecked(False)
        self.checkBox_svcLinear.setChecked(False)
        self.checkBox_svcPoly.setChecked(False)  # prediction page checks
        self.checkBox_gaussianNB.setChecked(False)
        self.checkBox_svcRBF.setChecked(False)
    ...
    def analyze_signal(self):
        if(self.selected_table_name != ""):
            ...
            Features.insertResults(self, matrix)
        else :
    ...
    def analyze_all_continuous(self):
        ...
        for table in list_tables:
            if (table.split('_')[0] != 'Comp'):
                ...
                if (functionNames.get(func_name, -1) == -1):
                    functionNames[str(func_name)] = iteration
                    iteration += 1

        accuracy_Mat = [['' for x in range(7)] for x in range(iteration)]
        recall_Mat = [['' for x in range(7)] for x in range(iteration)]
        precision_Mat = [['' for x in range(7)] for x in range(iteration)]
        ...
        for table in list_tables:
            ...
            if (table.split('_')[0] != 'Comp'):
                ...
                string = accuracy_Mat[functionNames.get(func_name)]
                array = [func_name, result[0][2], result[1][2], result[2][2], result[3][2], result[4][2], result[5][2]]
                if (string[0] == ''):
                    accuracy_Mat[functionNames.get(func_name)] = array
                else:
                    arr = [a + '-' + b for a, b in zip(string, array)]
                    accuracy_Mat[functionNames.get(func_name)] = arr

                array = [func_name, result[0][3], result[1][3], result[2][3], result[3][3], result[4][3], result[5][3]]
                if (string[0] == ''):
                    precision_Mat[functionNames.get(func_name)] = array
                else:
                    arr = [a + '-' + b for a, b in zip(string, array)]
                    precision_Mat[functionNames.get(func_name)] = arr

                array = [func_name, result[0][4], result[1][4], result[2][4], result[3][4], result[4][4], result[5][4]]
                if (string[0] == ''):
                    recall_Mat[functionNames.get(func_name)] = array
                else:
                    arr = [a + '-' + b for a, b in zip(string, array)]
                    recall_Mat[functionNames.get(func_name)] = arr

        avg_acc_matrix = cont_create_tables(accuracy_Mat)
        avg_rec_matrix = cont_create_tables(recall_Mat)
        avg_prec_matrix = cont_create_tables(precision_Mat)
...
def create_tables(matrix):
        ...
        if (symiter != 0):
            ...
            avg_acc_matrix[0] = create_average(avgSym, 'sym')
        if (haariter != 0):
            avgHaar = [x / (haariter / 6) for x in arrayHaar]
            avg_acc_matrix[1] = create_average(avgHaar, 'haar')
        if (coifiter != 0):
            avgCoif = [x / (coifiter / 6) for x in arrayCoif]
            avg_acc_matrix[2] = create_average(avgCoif, 'coif')
        if (dbiter != 0):
            avgDb = [x / (dbiter / 6) for x in arrayDb]
            avg_acc_matrix[3] = create_average(avgDb, 'db')
        if (rbioiter != 0):
            avgRbio = [x / (rbioiter / 6) for x in arrayRbio]
            avg_acc_matrix[4] = create_average(avgRbio, 'rbio')
        if (bioriter != 0):
            avgBior = [x / (bioriter / 6) for x in arrayBior]
            avg_acc_matrix[5] = create_average(avgBior, 'bior')
        if (dmeyiter != 0):
            avgDmey = [x / (dmeyiter / 6) for x in arrayDmey]
            avg_acc_matrix[6] = create_average(avgDmey, 'dmey')

        return avg_acc_matrix

def create_average(result, func_name):
    i= 0
...
def cont_create_tables(matrix):
    ...
    if (cgauiter != 0):
        ...
        avg_acc_matrix[0] = create_average_comp(avgCgau, 'cgau')
    if (morliter != 0):
        avgMorl = [x / (morliter / 6) for x in arrayMorl]
        avg_acc_matrix[1] = create_average_comp(avgMorl, 'morl')
    if (cmoriter != 0):
        avgCmor = [x / (cmoriter / 6) for x in arrayCmor]
        avg_acc_matrix[2] = create_average_comp(avgCmor, 'cmor')
    if (fbspiter != 0):
        avgFbsp = [x / (fbspiter / 6) for x in arrayFbsp]
        avg_acc_matrix[3] = create_average_comp(avgFbsp, 'fbsp')
    if (mexhiter != 0):
        avgMexh = [x / (mexhiter / 6) for x in arrayMexh]
        avg_acc_matrix[4] = create_average_comp(avgMexh, 'mexh')
    if (gausiter != 0):
        avgGaus = [x / (gausiter / 6) for x in arrayGaus]
        avg_acc_matrix[5] = create_average_comp(avgGaus, 'gaus')
    if (shaniter != 0):
        avgShan = [x / (shaniter / 6) for x in arrayShan]
        avg_acc_matrix[6] = create_average_comp(avgShan, 'shan')

    return avg_acc_matrix

def create_average_comp(result, func_name):
    i= 0
    array = [func_name, str("%.2f" % result[0]) , str("%.2f" % result[1]) , str("%.2f" % result[2]) ,
                        str("%.2f" % result[3]) , str("%.2f" % result[4]) , str("%.2f" % result[5])]
    return array
