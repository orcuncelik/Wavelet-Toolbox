...
def message(info, type):   #MessageBox
    msg = QMessageBox()
...
def checked_functions(self):    #Öğrenme algoritmasını kontrol ediyor
    functions = []
    if self.checkBox_randomForest.isChecked():
        functions.append(1)
    if self.checkBox_knn.isChecked():
        functions.append(2)
    if self.checkBox_svcPoly.isChecked():
        functions.append(3)
...
def discrete_check_wavelet(self):
    return self.comboBox_1D_Type.currentText(),self.comboBox_1D_Level.currentData()         # Wavelet tipi ve seviyesi kontrol ediliyor

def cont_check_wavelet(self):                                                                 #Continuous Wavelet için  scale kontrolü
...
def discrete_plot_signal(self):         #sinyal çizdiriliyor.
...
def init_table(self, row):              #tabloların oluşturulması
...
def addHeader(self):
    ...
    #Kontrolü yapılan istatistiksel fonksiyonlar için tablolara satır eklenmesi
    if (self.average):
        col += 1
    ...
    #Verilen Level için tablonun oluşturulması
...
def insertTable(self, row, col):
    ...
    for i in range(1, self.wavelet_level + 2):                      # Source sinyali dışındakileri dahil ediyoruz
        if (self.average):
...
def insertTableComplex(self, row, col):
    ...
    for i in range(1, self.wavelet_level + 2):                      #İlk sinyal source onu dahil etmiyoruz
        if (self.average):
            avg_results.append(average(self.signals[i]))
        if (self.entropy):
            entropy_results.append(ent(self.signals[i]))
...
#results tablosunun oluşturulması
