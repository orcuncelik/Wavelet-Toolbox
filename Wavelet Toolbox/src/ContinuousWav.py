...
class ContinuousWT(QMainWindow):
    ...
    def __init__(self):
        ...
        loadUi("../ui/Continuous.ui", self)
        self.setWindowTitle("Continuous Wavelet Analysis")
        ...
        #self.addToolBar(NavigationToolbar(self.widget_1DCont_Original.canvas, self))
    ...
    def load_folder(self):
        ...
        if (self.load_check):
            ...
            if (len(self.all_signals) > 1):                                  #Çoklu Veri Girişi
                self.label_1DCont_DataName.setText("Multiple Signals")        # Veri Adı
            else:
                name = self.all_signals[0].split(sep='/')
                self.label_1DCont_DataName.setText(name[-1])                   # Veri adı değiştirme
        else:
    ...
    def save_to_db(self):
        if(self.load_check and self.analyze_check):
            ...
            Database.create_table(Database.database_name, table_name, self.db_header, "")                   #İstatistiksel fonksiyonlar ile yeni tablo oluşturma
